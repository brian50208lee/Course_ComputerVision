...
public class ImgUtil {
	...
	public static int[][] imgBinarizeMatrix(BufferedImage bi ,int threshold){
		BufferedImage source= toGrayImage(bi);
		...
		for (int y = 0; y < source.getHeight(); y++) {
			for (int x = 0; x < source.getWidth() ; x++) {
				...
				//System.out.printf("%x ",rgb);
				int gray=r;
				int newBinarizeValue = gray >= threshold? 1: 0;
				result[y][x] = newBinarizeValue;
			}
		}
		return result;
	}
	...
	public static void show2DMatrix(int matrix[][],String title){
		BufferedImage bi = new BufferedImage(matrix[0].length, matrix.length, BufferedImage.TYPE_INT_ARGB);
	...
	public static BufferedImage downsample(BufferedImage bi ,int smpBlockHeight, int smpBlockWidth){
		...
		int resultX = 0;
		int resultY = 0;
		BufferedImage result = new BufferedImage(bi.getWidth() / smpBlockWidth, bi.getHeight() / smpBlockHeight, bi.getType());
